...
async def get_dishes(submenu_id: UUID, db: Session = Depends(get_session), rd: Redis = Depends(get_redis)):
    return await dishService.get_dishes(submenu_id, db, rd)


@router.get('/{menu_id}/submenus/{submenu_id}/dishes/{id}')
async def get_dish(id: UUID, db: Session = Depends(get_session), rd: Redis = Depends(get_redis)):
